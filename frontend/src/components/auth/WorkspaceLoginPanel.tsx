...
import { ArrowLeft, Building2, Info, Lock, Send, Unlock, UserPlus } from 'lucide-react'
...
export type WorkspaceLoginPanelProps = {
  workspaceSlug: string
  /** Pagina completa (rota /w/:slug/login) ou somente o formulario (ao lado da lista). */
  variant: 'page' | 'embedded'
}

export function WorkspaceLoginPanel({ workspaceSlug, variant }: WorkspaceLoginPanelProps) {
  ...
  const workspaceMeta = useMemo(() => {
    ...
    const description =
      workspace.slug === PILOT_WORKSPACE_SLUG
        ? 'Ambiente de referencia da Central de Tarefas, alinhado a operacao atual da base.'
        : workspace.description?.trim() || fallback
    return {
  ...
  const accessAlert =
    currentUser && workspaceAccess.status !== 'idle' && workspaceAccess.status !== 'loading'
      ? {
          severity:
            workspaceAccess.status === 'blocked' ||
            workspaceAccess.status === 'not_found' ||
            workspaceAccess.status === 'error'
              ? 'warning'
              : 'info',
          message: workspaceAccess.message,
        }
      : variant === 'page' && locationState?.accessMessage
        ? {
            severity:
              locationState.accessStatus === 'blocked' ||
              locationState.accessStatus === 'not_found' ||
              locationState.accessStatus === 'error'
                ? 'warning'
                : 'info',
  ...
  const titleVariant = variant === 'embedded' ? 'h4' : 'h3'
  const formSurface = (
    <AppSurface
      sx={{
        width: '100%',
        maxWidth: variant === 'embedded' ? '100%' : 520,
        mx: variant === 'embedded' ? 0 : 'auto',
      }}
    >
      {workspaceLoading ? (
        ...
        <Stack spacing={2.25}>
          <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between" sx={{ flexWrap: 'wrap' }}>
            <Box>
              <Typography variant={titleVariant} sx={{ mb: 0.35, fontWeight: 800, letterSpacing: '-0.02em' }}>
                {workspaceMeta?.title}
              </Typography>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            <Stack spacing={2}>
              ...
              {definePasswordMode && !requestMode ? (
                <Alert severity="info">Primeiro acesso detectado. Defina a senha forte da sua conta para continuar.</Alert>
              ) : null}
          ...
          <Stack direction="row" spacing={1} alignItems="flex-start">
            ...
            <Typography variant="body2" color="text.secondary">
              {variant === 'embedded' ? (
                <>
                  Contexto: <strong>{workspaceMeta?.title}</strong>. Troque de workspace pela lista à esquerda quando
                  precisar.
                </>
              ) : (
                <>
                  Você está entrando em <strong>{workspaceMeta?.title}</strong>. Se quiser trocar de área, volte para a
  ...
  return (
    ...
    >
      ...
      >
        <Button variant="text" color="inherit" startIcon={<ArrowLeft size={16} />} onClick={handleBackToWorkspaces}>
          Workspaces
        </Button>
